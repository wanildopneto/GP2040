...
// returns a bitmap of devices which are present (128 bits = 16 bytes, LSB
// first) A set bit indicates that a device responded at that address
//
...
int I2CWrite(BBI2C *pI2C, uint8_t iAddr, uint8_t *pData, int iLen) {
  ...
  rc = i2c_write_blocking(pI2C->picoI2C, iAddr, pData, iLen,
...
int I2CReadRegister(BBI2C *pI2C, uint8_t iAddr, uint8_t u8Register,
  ...
  rc = i2c_write_blocking(pI2C->picoI2C, iAddr, &u8Register, 1,
                          true);  // true to keep master control of bus
  if (rc >= 0) {
...
int I2CDiscoverDevice(BBI2C *pI2C, uint8_t i) {
  ...
  if (i >= 0x40 && i <= 0x4f)  // check for TI INA219 power measurement sensor
  {
    ...
    if (cTemp[0] == 0x39 && cTemp[1] == 0x9f) return DEVICE_INA219;
  }
  ...
  // try to identify it from the known devices using register contents
  {
    ...
    if (cTemp[0] == 0x10 && cTemp[1] == 0x50) return DEVICE_HDC1080;
    ...
    if (cTemp[0] == 0x0 &&
        cTemp[1] == 0x84)  // manufacturer code + software revision
      return DEVICE_LM8330;

    // Check for MAX44009
    if (i == 0x4a || i == 0x4b) {
      for (j = 0; j < 8; j++)
        I2CReadRegister(pI2C, i, j, &cTemp[j],
                        1);  // check for power-up reset state of registers
    ...
    I2CReadRegister(pI2C, i, 0x03, &cTemp[2],
                    2);  // Hi_thresh defaults to 0x7fff
    if (cTemp[0] == 0x80 && cTemp[1] == 0x00 && cTemp[2] == 0x7f &&
        cTemp[3] == 0xff)
      return DEVICE_ADS1115;

    // Check for MCP9808
    I2CReadRegister(pI2C, i, 0x06, cTemp,
                    2);  // manufacturer ID && get device ID/revision
    I2CReadRegister(pI2C, i, 0x07, &cTemp[2],
                    2);  // need to read them individually
    ...
    if (cTemp[0] == 0x69) return DEVICE_LSM6DS3;

    // Check for ADXL345
    I2CReadRegister(pI2C, i, 0x00, cTemp, 1);  // DEVID
    if (cTemp[0] == 0xe5) return DEVICE_ADXL345;
